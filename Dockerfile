...
RUN yum -y install http://repos.mesosphere.com/el/7/noarch/RPMS/mesosphere-el-repo-7-1.noarch.rpm \
    epel-release  \
    wget && \
    yum update -y
...
RUN mkdir ${JENKINS_WARDIR} && chown jenkins:jenkins ${JENKINS_WARDIR}
...
RUN tar -xvzf ${JENKINS_WARDIR}/mesos-${MESOS_JENKINS_VERSION}.tar.gz && \
